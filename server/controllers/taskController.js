...
const deleteRestoreTask = asyncHandler(async (req, res) => {
  try {
    const { id } = req.params;       // will be undefined if route = /delete-restore
    const { actionType } = req.query;
    ...
    if (id) {
      ...
      if (actionType === "delete") {
        ...
        return res.status(400).json({ status: false, message: "Invalid actionType for single task" });
      }
    } else {
      // 🔹 Bulk Actions
      if (actionType === "deleteAll") {
        ...
        return res.status(400).json({ status: false, message: "Invalid actionType for bulk operation" });
      }
    }
